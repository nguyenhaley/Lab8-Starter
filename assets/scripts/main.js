...
function initializeServiceWorker() {
  ...
  /*******************/
  // We first must register our ServiceWorker here before any of the code inv
  // sw.js is executed.
    ...
    window.addEventListener('load', (event) => {
      ...
      try{
        ...
        if(registration){
          ...
          console.log("Something went wrong witht he service worker registration :( ");
        }
  ...
  return new Promise (async (resolve, reject) => {
    ...
    for(let i = 0; i < RECIPE_URLS.length; i ++){
      ...
      try{
        ...
        let urlFectched = await fetch(RECIPE_URLS[i]);
        // A7. TODO - For each fetch response, retrieve the JSON from it using .json().
          ...
          //            "await" again
        let response = await urlFectched.json();
        // A8. TODO - Add the new recipe to the recipes array
        fetchedRecipes.push(response);
